...
def getGCFromFile(path):
    ...
    directory = path + 'Data/'
    for path2, dirs, files in os.walk(directory):
        for chr in dirs:
            if 'chr' in chr and chr != 'chrAll':
                filename = path2+chr+'/Shuffle_'+chr+'_location.fas'
                df = df.append(importData(filename))
                filename = path2+chr+'/Shuffle_'+chr.split('r')[1]+'_junction.fas'
                df = df.append(importData(filename))
    df = sumNt(df)
    return(df)
...
def main(path):
	...
	:type path: string
    """
    pG4rFileShuf = path+'Results/All/pG4r_shuffle.csv'
    pG4rFileWt = path+'Results/All/HS_All_G4InTranscript.txt'
    df = getGCFromFile(path)
